...
function landing(): string {
  return `${header()}<main id="main" tabindex="-1">${messages()}
    ...
    <section class="limits-section" aria-labelledby="limits-title">
      ...
      <div class="measure"><p>Microphone mode looks for separate attacks above the recent sound level. Use tap input if it adds unwanted marks.</p><p>Steady Take does not identify notes, assess technique, or replace a teacher. You can correct any take before saving.</p><p>No recording is kept. Only attack times and your passage history are stored on this device.</p></div>
    </section>
