# Landing copy audit

Audit date: 2026-08-28. Counts split hyphenated terms as one word. Interface
labels and fragments are included after the complete sentences. No sentence
exceeds 22 words. No banned word appears.

| Landing sentence | Words |
| --- | ---: |
| For beginning instrumentalists who want consistent timing across a short passage. | 10 |
| See six sessions and a 52% drop in timing spread. | 10 |
| Audio stays on this device. | 5 |
| Works offline after the first visit. | 6 |
| Free for one saved passage. | 5 |
| Uneven attacks settle into a repeatable shape. | 7 |
| Steady Take compares the gaps between your attacks. | 8 |
| Smaller spread means your repetitions align more closely. | 8 |
| Name it, choose the tempo, and set its attack count. | 10 |
| Use your microphone, a MIDI note, or the large tap key. | 11 |
| Mark controlled takes and watch the same passage over time. | 10 |
| Microphone onset detection works best with clean, separate attacks. | 9 |
| Room noise can add false marks. | 6 |
| Steady Take does not identify notes, assess technique, or replace a teacher. | 12 |
| You can correct any take before saving. | 7 |
| No recording is kept. | 4 |
| Only onset times and your passage history are stored on this device. | 11 |
| Practice one passage free. | 4 |
| The full version adds unlimited saved passages and complete history. | 9 |
| Sociobot is the merchant of record. | 6 |
| Purchase terms apply. | 3 |

Key fragments: “Measure steadier practice takes” (4), “Try it with sample
data” (5), “See spread, not a grade” (6), “Repeat one short passage” (4), “A
timing mirror, not a judge” (6), and “Keep every passage” (3).

## Terminology

| Concept | One term used |
| --- | --- |
| A learner-selected exercise | passage |
| One performance of that passage | take |
| Six takes measured together | session |
| Difference across matched attacks | timing spread |
| A learner's own quality marker | controlled |
| Paid state | full version |
